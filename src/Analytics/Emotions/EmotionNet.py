...
def EmotionModel(model_path, num_of_classes, num_of_channels=1, device=device):
    # load the emotionNet weights
    model = EmotionNet(num_of_channels=num_of_channels, num_of_classes=num_of_classes)

    # Load the pre-trained weights
    model_weights = torch.load(model_path, map_location=torch.device(device=device))
