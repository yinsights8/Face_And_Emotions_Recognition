...
class FaceIdentity:
    ...
    def RealTimeRecognition(self, getEmotions=False, getFaceLoc=False):
        ...
        if not success:
            logging.info(f"[{__name__}] Camera could not start...")
            return None

        print("Starting RealTimeRecognition")
        logging.info(f"INFO[{__name__}] Frames have been read")
        ...
        logging.info(f"INFO[{__name__}] Detection results...")
        ...
        if len(results) != 0:
            ...
            for result in TrackerResults:
                ...
                if face_bbox is not None:
                    ...
                    print(f"Cosine Score: {cosineScore}, Name: {Name}")
                    ...
                    if Name is not None:
                        ...
                        if getFaceLoc:
                            cvz.cornerRect(img=FaceFromCamera, bbox=(int(x1), int(y1), int(x2 - x1), int(y2 - y1)), l=20, t=3,
                                           colorR=RectColor)
